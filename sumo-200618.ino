...
//VALUES
int Speed =70                   80; // Idle Speed while no sensor giving data.
...
if (digitalRead(MSens)==LOW) {Set_Motor(MaxSpeed, MaxSpeed,1); digitalWrite(Buzzer, HIGH); LastValue=5;
}else if (digitalRead(LSens)== LOW) {Set_Motor(-40, TurnSpeed,1); digitalWrite(Buzzer, HIGH); LastValue=7;
}else if (digitalRead(RSens)==LOW) {Set_Motor(TurnSpeed, -40,1); digitalWrite(Buzzer, HIGH); LastValue=3;
}else if (digitalRead(LFSens)== LOW) {Set_Motor(-40, TurnSpeed,1); digitalWrite(Buzzer, HIGH); LastValue=7;
}else if (digitalRead(RFSens)==LOW) {Set_Motor(TurnSpeed, -40,1); digitalWrite(Buzzer, HIGH); LastValue=3;
}else{
  digitalWrite(Buzzer, LOW);
  Speed=(analogRead(SPD)/10.3); Speed=100-Speed; 
  
      if(LastValue==5) { 
          Set_Motor(35, 35,1);       // Forward, Based on SPD (A7) Trimpot
          
        }else if(LastValue==7){ 
          Set_Motor(-20, Speed,2);   // Left Turning Based on SPD (A7) Trimpot
          
        }else if (LastValue==3){ 
          Set_Motor(Speed, -20,2);   // Right Turning Based on SPD (A7) Trimpot
